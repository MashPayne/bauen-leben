...
/**
 * Class wdgShippingOptions
 *
 * Provide methods to handle input field "shippingOptions".
 * @copyright  Stefan Gandlau 2009
 * @author     Stefan Gandlau <stefan@gandlau.net>
 * @package    webShop
 */
  class wdgShippingOptions extends Widget {
 ...
 /**
  ...
  public function validate()
  {
    ...
    public function generate() {
      ...
      switch($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']['mode']) {
      	case 'byPrice': {
      		return $this->generateByPrice();
      	} break;
				case 'byWeight': {
					return $this->generateByWeight();
				} break;
      }

    }
    
    protected function generateByPrice() {
      ...
      $return .= '<table cellspacing="0" cellpadding="0" class="tl_optionwizard" id="ctrl_'.$this->strId.'" summary="Field wizard">
        <thead>
          <tr>
            <th>'.$GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']['reference']['lblSum'] .'</th>
            <th>'.$GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']['reference']['lblPrice'].'</th>
            <th></th>
            <th></th>
          </tr>
        </thead>
    ...
    protected function generateByWeight() {
	    $arrButtons = array('copy', 'up', 'down', 'delete');
	    $strCommand = 'cmd_' . $this->strField;
      ...
      $return .= '<table cellspacing="0" cellpadding="0" class="tl_optionwizard" id="ctrl_'.$this->strId.'" summary="Field wizard">
        <thead>
          <tr>
            <th>'.$GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']['reference']['lblWeight'] .'</th>
            <th>'.$GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']['reference']['lblPrice'].'</th>
            <th></th>
			      <th></th>
			    </tr>
